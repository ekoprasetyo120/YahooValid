...
$host = [
	"yahoo.com","yahoo.co.id","yahoo.co.uk",
	"hotmail.com"
];
$host = implode("|", $host);

$list = readline("Insert list : ");
$emails = file_get_contents($list);
$emails = explode("\n", $emails);

foreach ($emails as $em){
	if (cek(preg_replace("($host)", "", str_replace("@", "",$em))) == "Email Hidup" ){
		echo "$em [\033[38;5;10mVALID\033[0m]\n";
		$res = "result.txt";
		$txt = $em . "\n";
		$result = fopen("hasil.txt", "a+");
		fwrite($result, $txt);
		fclose($result);
	} else {
		echo "$em [\033[38;5;9mINVALID\033[0m]\n";
	}
}
